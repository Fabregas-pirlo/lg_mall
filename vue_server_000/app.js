...
var sql = "SELECT id FROM lg_cart";
...
server.get("/carts",(req,res)=>{
   ...
   //(2)创建sql语句 
   var sql = "SELECT id,lname,price,count FROM ";
   sql+=" lg_cart WHERE uid = ?";
...
server.get("/delItems",(req,res)=>{
  ...
  //(2)sql 删除多个购物车中id
  var sql = `DELETE FROM lg_cart WHERE id IN (${ids})`;
  //(3)json
  pool.query(sql,(err,result)=>{
